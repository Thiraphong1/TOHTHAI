...
exports.changeStatus = async (req, res) => {
  try {
    const { id, enabled } = req.body; // ต้องได้ { id, enabled }
...
exports.saveOrder = async (req, res) => {
  try {
    ...
    if (!userCart || userCart.products.length === 0) {
      return res.status(400).json({ ok: false, message: "No cart" });
    }

    // Step 2: ตรวจสอบจำนวนสินค้า
